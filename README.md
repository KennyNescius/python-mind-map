...
2. **Включите Identity:** Site settings → Identity → Enable.
   ...
   - *(опционально)* External providers → включить **Google**, чтобы входить без
     пароля. Входите Google-аккаунтом с тем же email, на который выслано
     приглашение.
...
Ссылка из письма-приглашения (или OAuth-редирект Google) приземляется на корень
сайта с токеном в адресе. Приложение само подхватывает этот токен, показывает
форму установки пароля / завершения входа и затем перенаправляет на `/edit`.
Дальше: редактируете карту и жмёте «Сохранить» — изменения уезжают в репозиторий,
Netlify пересобирает сайт, и обновлённая карта доступна всем на чтение.
